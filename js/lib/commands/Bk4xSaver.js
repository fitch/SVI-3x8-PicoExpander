...
class Bk4xSaver {
    static async save(filename = null, picoAddress = null, onComplete = null, onError = null) {
        if (!filename) {
            filename = path.join(process.cwd(), 'saved_bk4x.bin');
        }
            ...
            try {
                if (waitingForOK) {
                    ...
                    if (cmd === 'OK') {
                        ...
                        console.error("BK4X save failed - another command is in progress. Please try again.");
                        client.end();
                    } else if (cmd === 'ER') {
            ...
            if (totalReceived < expectedSize && !waitingForOK) {
                ...
                if (chunks.length > 0) {
                    try {
                        ...
                        console.log(`\nPartial BK4X data saved to: ${filename} (${receivedData.length} bytes)`);
                    } catch (writeErr) {
                        console.error(`Error writing file: ${writeErr.message}`);
                    }
                }
        ...
        client.write(createCommandBuffer("SB", 0, 0));
    }
}

module.exports = Bk4xSaver;
